...
dgl <- dgl[aveLogCPM(dgl) >= 0,,keep.lib.sizes=FALSE]
...
EvsM <- makeContrasts(groupgroupE-groupgroupM, levels=dmat)
res <- glmQLFTest(fit, contrast=EvsM)
topTags(res)
topTags(lrtres[[1]])
is.de <- decideTestsDGE(res)
# is.de <- decideTestsDGE(lrtres[[1]])
summary(is.de)
...
par(mfcol=mfcol)
for(k in 1:npanel) {
hist(PV[,k],n=100 ,xlab="P-value", main=colnames(logFC)[k], cex.main=0.8)
}
dev.off()
...
for (k in 1:wn) {
 ...
 # write.table(restab, file=file.path(outpath,paste(sub_analyse,'_all.txt', sep="")), quote=F, row.names=F, sep='\t')
...
for (chrom in c('2_', '3_', '4_', 'XL_', 'XR_', 'X') ) {
	...
	pdf(file.path(outpath, paste('Chisq_Chr_', chrom, 'FDR_', FDR2use, '_logFC_', logFC_use, '_', paste(colnames(cmat)[k]), '.pdf', sep="")), width=16, height=6)
	par(mfrow=c(1,3))
...
### Estimation of X counts
	...
	down_XR <- sum(list_de[[k]][[paste('logFC.', colnames(cmat)[k], sep="")]][grepl('XR_', list_de[[k]]$chr)] < 0, na.rm=T) 
		...
		pasteX <- paste('chisq =', round(chi_test$p.value, digits=2), sep=" ")
		pdf(file.path(outpath, paste('Chisq_Chr_', chrom,'vs_XR_FDR_', FDR2use, '_logFC_', logFC_use, '_', paste(colnames(cmat)[k]), '.pdf', sep="")), width=16, height=6)
		par(mfrow=c(1,3))
		par(mar=c(5,5,4,3), oma=c(0,0,2,0))
		mp <- barplot(chi_obs_data, col=c(cbgreen,cbblue), main = paste('DE ',(colnames(cmat)[k]), '\nFDR=',FDR2use, ' logFC=', logFC_use, sep=""), beside=T, cex.main=1.4, cex.lab=1.2, ylab="Gene number")
		if (chi_test$p.value < 0.001) {title(xlab=pasteX, font.lab=2)} else {title(xlab=pasteX, font.lab=1)}
...
## Physical map
	for(k in 1:ncol(cmat)) {
	par(mar=c(5,5,4,3))
	pdf(file.path(outpath, paste('Chr_location_FDR_', FDR2use, '_logFC_', logFC_use, '_',colnames(cmat)[k], '.pdf', sep="")), width=16, height=8)
...
for(k in 1:ncol(cmat)) {
	...
	write.table(list_de[[k]]$gene, file=file.path(outpath, paste('gene_',FDR2use, '_', colnames(cmat)[k], '_', sub_analyse, '.txt', sep="")), quote=F, row.names=F, sep='\t')

	write.table(data.frame(gene=FDR05_unbiased$gene, logFC=FDR05_unbiased[[paste('logFC.',colnames(cmat)[k], sep="")]], FDR=FDR05_unbiased[[paste('FDR.',colnames(cmat)[k], sep="")]]), file=file.path(outpath, paste('unbiased_',FDR2use, '_', colnames(cmat)[k], '_', sub_analyse, '.txt', sep="")), quote=F, sep='\t')
	write.table(data.frame(gene=UP$gene, logFC=UP[[paste('logFC.',colnames(cmat)[k], sep="")]], FDR=UP[[paste('FDR.',colnames(cmat)[k], sep="")]]), file=file.path(outpath, paste('UP_',FDR2use, '_', colnames(cmat)[k], '_', sub_analyse, '.txt', sep="")), quote=F, sep='\t')
	write.table(data.frame(gene=DOWN$gene, logFC=DOWN[[paste('logFC.',colnames(cmat)[k], sep="")]], FDR=DOWN[[paste('FDR.',colnames(cmat)[k], sep="")]]), file=file.path(outpath, paste('DOWN_',FDR2use, '_', colnames(cmat)[k], '_', sub_analyse, '.txt', sep="")), quote=F, sep='\t')
...
colnames(nc2) <- c('ID', paste(design$sample))
...
ggsave(file.path(outpath, paste('Violin_per_chr_all_', colnames(cmat)[k], '_', sub_analyse, '.pdf', sep="")), width=8, height=8)
...
p = ggplot(violinData, aes_string(factor(violinData$chrCombined), paste('logFC.', colnames(cmat)[k], sep="")), ylim)
p + geom_violin(scale="width", fill=16, trim=F) + geom_boxplot(width=0.15, col=4) + geom_hline(yintercept=-1,lty=3) + geom_hline(yintercept=1, lty=3) + geom_hline(yintercept=2, lty=2)+ geom_hline(yintercept=-2, lty=2) +  scale_y_continuous(breaks=c(-10, -5, -2, -1, 0, 1, 2, 5, 10)) + labs(title=paste('logFC.',colnames(cmat)[k], sep=""), x='Chromosome', y='logFC') + theme(panel.background = element_rect(fill='white'))  + theme(axis.text=element_text(size=12), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.title.x = element_text(colour = "black"), axis.title.y = element_text(colour = "black"))
ggsave(file.path(outpath, paste('Violin_per_chr_DE_',FDR2use, '_', colnames(cmat)[k], '_', sub_analyse, '.pdf', sep="")), width=8, height=8)
}


## New chisq test tables

library(sjPlot)

mapData2$chrom <- 'NA'
mapData2$chrom[mapData2$chr=='2_'] <- 'Chr_2'
mapData2$chrom[mapData2$chr=='3_'] <- 'Chr_3'
mapData2$chrom[mapData2$chr=='4_group1'] <- 'Chr_4'
mapData2$chrom[mapData2$chr=='4_group2'] <- 'Chr_4'
mapData2$chrom[mapData2$chr=='4_group3'] <- 'Chr_4'
mapData2$chrom[mapData2$chr=='4_group4'] <- 'Chr_4'
mapData2$chrom[mapData2$chr=='4_group5'] <- 'Chr_4'
mapData2$chrom[mapData2$chr=='XL_group1a'] <- 'Chr_XL'
mapData2$chrom[mapData2$chr=='XL_group1e'] <- 'Chr_XL'
mapData2$chrom[mapData2$chr=='XL_group3a'] <- 'Chr_XL'
mapData2$chrom[mapData2$chr=='XL_group3b'] <- 'Chr_XL'
mapData2$chrom[mapData2$chr=='XR_group3a'] <- 'Chr_XR'
mapData2$chrom[mapData2$chr=='XR_group5'] <- 'Chr_XR'
mapData2$chrom[mapData2$chr=='XR_group6'] <- 'Chr_XR'
mapData2$chrom[mapData2$chr=='XR_group8'] <- 'Chr_XR'

mapData3 <- subset(mapData2, mapData2$chrom!='NA') 

k <- 1

for(chromComp in c('Chr_2', 'Chr_3', 'Chr_4', 'Chr_XL', 'Chr_XR') ) {
	mapData3[[chromComp]] <- mapData3$chrom
	mapData3[[chromComp]][mapData3$chrom != chromComp] <- 'other'
}

mapDataXLXR <- subset(mapData3, mapData3$chrom!='2' & mapData3$chrom!='3'& mapData3$chrom!='4') 

sjt.xtab(mapData3$chrom, mapData3[[paste('FDR.', colnames(cmat)[k], sep="")]] < FDR2use, title=paste('Continency table ', colnames(cmat)[k], ' (FDR ', FDR2use, ') by Muller element', sep=""), show.row.prc=T, show.legend=T, show.summary=T, file=file.path(outpath, paste('Contingency_All_', colnames(cmat)[k], '_', FDR2use, '.html', sep='')), remove.spaces=T)

sjt.xtab(mapData3[['Chr_2']], mapData3[[paste('FDR.', colnames(cmat)[k], sep="")]] < FDR2use, title=paste('Continency table ', colnames(cmat)[k], ' (FDR ', FDR2use, ') 2 vs rest', sep=""), show.row.prc=T, show.legend=T, show.summary=T, file=file.path(outpath, paste('Contingency_2_', colnames(cmat)[k], '_', FDR2use, '.html', sep='')), remove.spaces=T)

sjt.xtab(mapData3[['Chr_3']], mapData3[[paste('FDR.', colnames(cmat)[k], sep="")]] < FDR2use, title=paste('Continency table ', colnames(cmat)[k], ' (FDR ', FDR2use, ') 3 vs rest', sep=""), show.row.prc=T, show.legend=T, show.summary=T, file=file.path(outpath, paste('Contingency_3_', colnames(cmat)[k], '_', FDR2use, '.html', sep='')), remove.spaces=T)
sjt.xtab(mapData3[['Chr_4']], mapData3[[paste('FDR.', colnames(cmat)[k], sep="")]] < FDR2use, title=paste('Continency table ', colnames(cmat)[k], ' (FDR ', FDR2use, ') 4 vs rest', sep=""), show.row.prc=T, show.legend=T, show.summary=T, file=file.path(outpath, paste('Contingency_4_', colnames(cmat)[k], '_', FDR2use, '.html', sep='')), remove.spaces=T)
print('printer2')

sjt.xtab(mapData3[['Chr_XL']], mapData3[[paste('FDR.', colnames(cmat)[k], sep="")]] < FDR2use, title=paste('Continency table ', colnames(cmat)[k], ' (FDR ', FDR2use, ') XL vs rest', sep=""), show.row.prc=T, show.legend=T, show.summary=T, file=file.path(outpath, paste('Contingency_XL_', colnames(cmat)[k], '_', FDR2use, '.html', sep='')), remove.spaces=T)
sjt.xtab(mapData3[['Chr_XR']], mapData3[[paste('FDR.', colnames(cmat)[k], sep="")]] < FDR2use, title=paste('Continency table ', colnames(cmat)[k], ' (FDR ', FDR2use, ') XR vs rest', sep=""), show.row.prc=T, show.legend=T, show.summary=T, file=file.path(outpath, paste('Contingency_XR_', colnames(cmat)[k], '_', FDR2use, '.html', sep='')), remove.spaces=T)
print('printer2')
sjt.xtab(mapDataXLXR$chrom, mapDataXLXR[[paste('FDR.', colnames(cmat)[k], sep="")]] < FDR2use, title=paste('Continency table ', colnames(cmat)[k], ' (FDR ', FDR2use, ') XR vs XL', sep=""), show.row.prc=T, show.legend=T, show.summary=T, file=file.path(outpath, paste('Contingency_XR_XL_', colnames(cmat)[k], '_', FDR2use, '.html', sep='')))
...
if (nrow(DE_counts_relevant) > 2) {
pdf(file.path(outpath, paste('Heatmap_robustDE_', FDR2use, '_DE_', colnames(cmat)[k], '_', sub_analyse, '.pdf', sep="")), width=8, height=8)
